...
function getFontSize(font: string): number {
  ...
  return match ? parseInt(match[1] ?? "0", 10) : 30;
}
...
class Media {
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          ...
          color.rgb = mix(color.rgb, vec3(0.2), uBlur);
  ...
  update(
    ...
    // Apply blur effect to images on the ends
...
class App {
    ...
    const defaultItems = [
      {
        ...
        {
            image: `./gallery-images/7.png`,
            text: "Canada Parliament",
        },
        {
  ...
  onTouchDown(e: MouseEvent | TouchEvent) {
    ...
    this.start = "touches" in e ? e.touches?.[0]?.clientX ?? 0 : e.clientX ?? 0;
  }

  onTouchMove(e: MouseEvent | TouchEvent) {
    if (!this.isDown) return;
    const x = "touches" in e ? e.touches?.[0]?.clientX ?? 0 : e.clientX ?? 0;
